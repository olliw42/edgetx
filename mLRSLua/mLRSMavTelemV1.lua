--local toolName = "TNS|mLRS MavTelem V1|TNE"
----------------------------------------------------------------------
...
-- returns a msg_frame
local function mavlinkMsgEncode(msg_struct, data)
...
debugAdd(string.format("MAV %d/%d seq=%d", msg.sysid, msg.compid, msg.seq))            
            ...
            if msg.crc_validated then -- crc validated
                ...
                mavRxCrcErrorCount = mavRxCrcErrorCount + 1
            end    
        end
    end
end
...
local function mavlinkSend(msg_struct, data)
    local msg_frame = mavlinkMsgEncode(msg_struct, data)
